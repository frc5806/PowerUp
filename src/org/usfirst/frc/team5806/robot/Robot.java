...
//		ahrs.resetDisplacement();
	...
	public void normalize() {
		...
		for (int j = 1; j < pathLeft.length; j += 4) {
			normalLeft.add(pathLeft[j]);
		}
		for (int j = 1; j < pathRight.length; j += 4) {
	...
	public void spline() throws IOException {
		...
		//String pathOrig = "C://Documents//derp2.txt";
		normalize();
		
		leftencoder.reset();
		rightencoder.reset();
		Map<Double, ArrayList<Double>> leftPath = getPathMap(pathLeft);
		Map<Double, ArrayList<Double>> rightPath = getPathMap(pathRight);
		...
		double elapsedTime;
		double timeStartSeconds = timeStart/1000.0;
		/*
		do {
			elapsedTime = System.currentTimeMillis();
			leftVolt = leftPath.get((elapsedTime/10)/100.0).get(0)/(14.0*1.3)+.3;
			
		} while(elapsedTime < 3000);
		*/
		///*for(int i=1; i < normalLeft.size(); i+= 3) {
			//long difference = System.currentTimeMillis() - timeStart;
			//ArrayList<ArrayList<Double>> path = Util.stringToPath236(Util.readStringFromFile(file));
			//leftVolt = path.get(1).get(2*i+1);
			//rightVolt = path.get(2).get(2*i+1);
		double minVolt = 0.3;
		double MIN_SPEED_UP = 0.0;
		double MIN_SPEED_DOWN = 0.0;
		double k1 = 1.0/(9.0*1.35);
		double k2 = 0.3;
		double k3 = 0.2;
		...
		double ticksTraveledScaled = 0.0;
		double avgDist = normalLeft.get(normalLeft.size()-1)/2.0 + normalRight.get(normalRight.size()-1)/2.0;
		double RAMP_UP_TICKS = 200*300*TICKS_SCALE;
		double maxSpeed = 0.0;
		...
		//double k4 = 0.08;
		double k4 = 0.0;
		for (int j = 1; j < normalLeft.size(); j += 4) {
			if (normalLeft.get(j) > maxSpeedLeft) maxSpeedLeft = normalLeft.get(j);
			if (normalRight.get(j) > maxSpeedRight) maxSpeedRight = normalRight.get(j);
		}
		maxSpeed = maxSpeedLeft/2.0 + maxSpeedRight/2.0;
		MIN_SPEED_UP = maxSpeed*MIN_SPEED_UP;
		MIN_SPEED_DOWN = maxSpeed*MIN_SPEED_DOWN;
			
		for (int i = 1; i < normalLeft.size(); i += 4) {
			//ticksTraveledScaled = (leftencoder.get()/2.0 + rightencoder.get()/2.0)*TICKS_SCALE;
			...
			/*
			if (i+2 <= normalLeft.size()) {
				leftAccel = (normalLeft.get(i+3)-normalLeft.get(i))/(normalLeft.get(i-1+3)-normalLeft.get(i-1));
				rightAccel = (normalRight.get(i+3)-normalRight.get(i))/(normalRight.get(i-1+3)-normalRight.get(i-1));
			}*/
			//System.out.println("% completed " + (i/(double)normalLeft.size()));
			
			/*
			if (ticksTraveledScaled > 0.0 && ticksTraveledScaled < RAMP_UP_TICKS) {
				leftSpeed = MIN_SPEED_UP + (normalLeft.get(i)-MIN_SPEED_UP)*(ticksTraveledScaled/RAMP_UP_TICKS);
				rightSpeed = MIN_SPEED_UP + (normalRight.get(i)-MIN_SPEED_UP)*(ticksTraveledScaled/RAMP_UP_TICKS);
			} else if (ticksTraveledScaled >= avgDist - RAMP_UP_TICKS) {
				leftSpeed = MIN_SPEED_DOWN + (normalLeft.get(i)-MIN_SPEED_DOWN)*(avgDist-ticksTraveledScaled)/(avgDist-RAMP_UP_TICKS);
				rightSpeed = MIN_SPEED_DOWN + (normalRight.get(i)-MIN_SPEED_DOWN)*(avgDist-ticksTraveledScaled)/(avgDist-RAMP_UP_TICKS);
			} else {
				leftSpeed = normalLeft.get(i);
				rightSpeed = normalRight.get(i);
			}
			*/
			...
			Timer.delay(0.025);
			robotdrive.tankDrive(k1*leftSpeed+k2*leftError+k3*leftAccel-k4*errorBetween+minVolt, k1*rightSpeed+k2*rightError+k3*rightAccel+k4*errorBetween+minVolt);
		}
		//}*/
		Timer.delay(0.05);
		robotdrive.tankDrive(0, 0);
